...
fn give_me_cookie(jar: &CookieJar<'_>) ->  RawHtml<&'static str> {
    ...
    let c0_vuln = Cookie::build(builder);
    let c1_vuln = Cookie::build(("cookie1_vuln", "value")).finish();
    let c1_2_vuln = Cookie::build(("cookie1_vuln", "value"));
    let c2_vuln = Cookie::build(("cookie2_vuln", "value")).path("/").finish();
    let c2_1_vuln = Cookie::build(("cookie2_1_vuln", "value")).path("\\").finish();
    let c3_vuln = Cookie::new("cookie3_vuln", "value"); // default path is broad
    let mut c4_vuln = Cookie::new("cookie4_vuln", "value");
    c4_vuln.set_path("/");
    let c5_vuln = Cookie::parse("c5_vuln=value;").expect("something went wrong");
    let c6_vuln = Cookie::parse_encoded("c6_vuln=val%20ue;path=/").expect("something went wrong");
    ...
    jar.add(c0_vuln); // sink - result
    jar.add(c1_vuln); // sink - result
    jar.add(c1_2_vuln); // sink - result
    ...
    let c1_safe = Cookie::build(("c1_safe", "value")).path("/specific/path").finish();
    let c1_2_safe = Cookie::build(("c1_safe", "value")).path("/specific/path");
    let mut c2_safe = Cookie::new("c2_safe", "value");
    c2_safe.set_path("/specific/path");
    let c3_safe = Cookie::parse("c3_safe=value; Path=/specific/path; HttpOnly").expect("something went wrong");
    jar.add(c1_safe); // sink - NOT a result, this is safe
    jar.add(c1_2_safe); // sink - NOT a result, this is safe
