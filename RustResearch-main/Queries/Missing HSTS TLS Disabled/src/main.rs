...
impl<'r> Responder<'r, 'static> for HSTS1 {
    fn respond_to(self, _: &'r rocket::Request<'_>) -> rocket::response::Result<'static> {
        ...
        response.raw_header("Strict-Transport-Security", "max-age=9999999999"); // SANITIZER
        response.ok()
    }
}
...
impl<'r> Responder<'r, 'static> for HSTS2 {
    fn respond_to(self, _: &'r rocket::Request<'_>) -> rocket::response::Result<'static> {
        let mut response = rocket::response::Response::build();
        response.raw_header("Strict-Transport-Security", "max-age=9999999999;includeSubDomains; preload"); // SANITIZER
        response.ok()
    }
}
...
fn rocket() -> _ {
    ...
    if args.len() == 1{
        if args[1] == "0" {
            // vulnerable to missing HSTS
            shield_to_be_used = missing_hsts::missing_hsts::get_vulnerable_shield_0();
        }
        else if args[1] == "1" {
            // vulnerable to missing HSTS
            shield_to_be_used = missing_hsts::missing_hsts::get_vulnerable_shield_1();
        }
        else if args[1] == "2" {
            // Vulnerable
            shield_to_be_used = missing_hsts::missing_hsts::get_vulnerable_shield_2();
        }
        else if args[1] == "3" {
            // SAFE to missing HSTS
            shield_to_be_used = missing_hsts::missing_hsts::get_safe_shield_3();
        }        
        else if args[1] == "4" {
            // SAFE to missing HSTS
            shield_to_be_used = missing_hsts::missing_hsts::get_safe_shield_4();
        }

        rocket::build()
            .attach(shield_to_be_used) // THIS IS SAFE/VULNERABLE DEPENDENT ON THE COMMAND LINE ARGUMENT
            .mount("/", routes![home])
            .mount("/", routes![custom_header]) // THIS IS ALWAYS SAFE
            .mount("/", routes![custom_header_2])  // THIS IS ALWAYS SAFE
    }
    else if args.len() > 2{
        ...
        rocket::build()
            ...
            .attach(rocket::fairing::AdHoc::on_response( // SANITIZER
                ...
                |_, res| {
                    Box::pin(async move {
                        res.set_raw_header("Strict-Transport-Security", "max-age=9999999999;includeSubDomains; preload");
                    })
                },
            ))
            .mount("/", routes![home]) // SAFE ALWAYS
            .mount("/", routes![custom_header]) // SAFE ALWAYS
            .mount("/", routes![custom_header_2]) // SAFE ALWAYS
    }
    else {
        ...
        print!("\n\n\n\nIN THE LAST ROCKET\n\n\n\n");
        rocket::build()
            .mount("/", routes![home]) // RESULT - MISSING HSTS
