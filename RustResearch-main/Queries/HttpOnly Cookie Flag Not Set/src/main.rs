...
fn give_me_cookie(jar: &CookieJar<'_>) ->  RawHtml<&'static str> {
    ...
    let c2_vuln = Cookie::build("cookie2_vuln", "value").http_only(false).finish();
    let c3_vuln = Cookie::new("cookie3_vuln", "value"); // default is not httpOnly
    let mut c4_vuln = Cookie::new("cookie4_vuln", "value");
    c4_vuln.set_http_only(false);
    let c5_vuln = Cookie::parse("c5_vuln=value;").expect("something went wrong");
    let c6_vuln = Cookie::parse_encoded("c6_vuln=val%20ue;").expect("something went wrong");
